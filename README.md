...
```bash
npm run dev:local # 加载 http://localhost:3000，并关闭桌面自动更新检查
...
本地联调前需先在 `http://localhost:3000` 启动 Web 服务。需要使用其他地址时，可设置
`SNACK_LOCAL_FRONTEND_URL`。涉及麦克风或录屏权限时，优先使用 `npm run build:local`，再打开
